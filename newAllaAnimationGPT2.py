...
def merge_animations(base_model_path, anims_folder_path, output_path):
    log("=" * 70)
    log("MERGE DE ANIMAÇÕES - PRESERVANDO RIG ORIGINAL")
    log("=" * 70)
    ...
    base_bone_names = get_armature_bone_names(base_armature)
    success_count = 0
    skipped_count = 0
    ...
    for obj in bpy.data.objects:
        if obj.type == 'MESH' and (obj.parent == base_armature or any(m.type == 'ARMATURE' for m in obj.modifiers)):
            obj.select_set(True)
    ...
    log(f"✅ Concluído com sucesso! {success_count} animações mescladas.")
